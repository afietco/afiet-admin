...
import { gscMock } from '../analytics/mock'
import { SERIES_COLORS, fmt, shortDate } from '../analytics/shared'
import { RANGES, type Range } from '../../services/analytics'

/**
 * Google Search Console performansı. Karar (31 Tem): servis hesabı + günlük
 * cron (Cloud Scheduler) GSC Search Analytics verisini Neon'a yazar; mülk
 * sc-domain:afiet.co. Backend bağlanana dek önizleme verisi gösterilir.
 */
...
const data = computed(() => gscMock(range.value))

const cards = computed(() => [
  { label: 'Tıklama', value: fmt(data.value.totals.clicks), tone: 'green', icon: 'pi pi-external-link', note: 'Google aramadan gelen' },
  { label: 'Gösterim', value: fmt(data.value.totals.impressions), tone: 'blue', icon: 'pi pi-eye', note: 'arama sonuçlarında görünme' },
  { label: 'CTR', value: `%${data.value.totals.ctrPct.toLocaleString('tr-TR')}`, tone: 'amber', icon: 'pi pi-percentage', note: 'tıklama / gösterim' },
  { label: 'Ort. pozisyon', value: data.value.totals.position.toLocaleString('tr-TR'), tone: 'coral', icon: 'pi pi-sort-numeric-down', note: 'küçük olan iyidir' },
])
...
<template>
  <div class="tab-body">
    <div class="seo-lead">
      ...
      <SelectButton v-model="range" :options="rangeOptions" option-label="label" option-value="value" :allow-empty="false" aria-label="Zaman aralığı" />
    </div>

    <div class="db-banner">
      <i class="pi pi-link" />
      <span>GSC bağlantısı henüz kurulmadı. Kurulum: 1) GCP'de servis hesabı açılır, 2) e-postası Search Console'da <strong>sc-domain:afiet.co</strong> mülküne okuma yetkili kullanıcı eklenir, 3) günlük cron veriyi çeker. Aşağısı örnek görünüm.</span>
    </div>
